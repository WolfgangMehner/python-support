"===============================================================================
"
"          File:  python-support.vim
"
"   Description:  Python IDE for Vim/gVim.
"
"                 See help file pythonsupport.txt .
"
"   VIM Version:  7.0+
...
if v:version < 700
	echohl WarningMsg
	echo 'The plugin python-support.vim needs Vim version >= 7.'
	echohl None
	finish
endif
...
" need compatible
if &cp || ( exists('g:Python_Version') && ! exists('g:Python_DevelopmentOverwrite') )
	finish
endif

let g:Python_Version= '0.5'     " version number of this script; do not change
...
let s:plugin_dir                = ''               " the directory hosting ftplugin/ plugin/ python-support/ ...
let s:Python_GlobalTemplateFile = ''               " the global templates, undefined for s:installation == 'local'
let s:Python_LocalTemplateFile  = ''               " the local templates
let s:Python_CustomTemplateFile = ''               " the custom templates

let s:Python_ToolboxDir         = []
...
if s:MSWIN
	...
	if match(      substitute( expand('<sfile>'), '\\', '/', 'g' ),
		...
		let s:Python_LocalTemplateFile  = s:plugin_dir.'/python-support/templates/Templates'
		let s:Python_CustomTemplateFile = $HOME.'/vimfiles/templates/python.templates'
		let s:Python_ToolboxDir        += [ s:plugin_dir.'/autoload/mmtoolbox/' ]
	else

		" system wide installation
		let s:installation              = 'system'
		let s:Python_GlobalTemplateFile = s:plugin_dir.'/python-support/templates/Templates'
		let s:Python_LocalTemplateFile  = $HOME.'/vimfiles/python-support/templates/Templates'
		let s:Python_CustomTemplateFile = $HOME.'/vimfiles/templates/python.templates'
		let s:Python_ToolboxDir        += [
	...
	if match( expand('<sfile>'), '\V'.resolve(expand('$HOME')) ) == 0
		...
		let s:installation              = 'local'
		let s:Python_LocalTemplateFile  = s:plugin_dir.'/python-support/templates/Templates'
		let s:Python_CustomTemplateFile = $HOME.'/.vim/templates/python.templates'
		let s:Python_ToolboxDir        += [ s:plugin_dir.'/autoload/mmtoolbox/' ]
	else
		...
		let s:installation              = 'system'
		let s:Python_GlobalTemplateFile = s:plugin_dir.'/python-support/templates/Templates'
		let s:Python_LocalTemplateFile  = $HOME.'/.vim/python-support/templates/Templates'
		let s:Python_CustomTemplateFile = $HOME.'/.vim/templates/python.templates'
		let s:Python_ToolboxDir        += [
					\	s:plugin_dir.'/autoload/mmtoolbox/',
...
let s:Python_LoadMenus             = 'auto'       " load the menus?
let s:Python_RootMenu              = '&Python'       " name of the root menu

if s:MSWIN
	let s:Python_OutputMethodList = [ 'vim-io', 'vim-qf', 'buffer' ]
else
	let s:Python_OutputMethodList = [ 'vim-io', 'vim-qf', 'buffer', 'xterm' ]
endif
let s:Python_OutputMethod          = 'vim-io'     " 'vim-io', 'vim-qf', 'buffer' or 'xterm'
let s:Python_DirectRun             = 'no'         " 'yes' or 'no'
let s:Python_LineEndCommColDefault = 49
let s:Lua_CommentLabel          = "BlockCommentNo_"
let s:Python_SnippetDir            = s:plugin_dir.'/python-support/codesnippets/'
let s:Python_SnippetBrowser        = 'gui'
let s:Python_UseToolbox            = 'yes'
let s:Python_AdditionalTemplates   = mmtemplates#config#GetFt ( 'python' )
...
if s:MSWIN
	let s:Python_BinPath = ''
else
	let s:Python_BinPath = ''
endif

call s:GetGlobalSetting ( 'Python_BinPath' )

if s:MSWIN
	let s:Python_BinPath = substitute ( s:Python_BinPath, '[^\\/]$', '&\\', '' )

	let s:Python_Executable = s:Python_BinPath.'python'       " python executable
	let s:Lua_CompilerExec = s:Python_BinPath.'luac'      " luac executable
else
	let s:Python_BinPath = substitute ( s:Python_BinPath, '[^\\/]$', '&/', '' )
...
call s:GetGlobalSetting ( 'Python_GlobalTemplateFile' )
call s:GetGlobalSetting ( 'Python_LocalTemplateFile' )
call s:GetGlobalSetting ( 'Python_CustomTemplateFile' )
call s:GetGlobalSetting ( 'Python_LoadMenus' )
call s:GetGlobalSetting ( 'Python_RootMenu' )
call s:GetGlobalSetting ( 'Python_OutputMethod' )
call s:GetGlobalSetting ( 'Python_DirectRun' )
call s:GetGlobalSetting ( 'Python_Executable' )
call s:GetGlobalSetting ( 'Lua_CompilerExec' )
call s:GetGlobalSetting ( 'Python_LineEndCommColDefault' )
call s:GetGlobalSetting ( 'Python_SnippetDir' )
call s:GetGlobalSetting ( 'Python_SnippetBrowser' )
call s:GetGlobalSetting ( 'Python_UseToolbox' )
call s:GetGlobalSetting ( 'Xterm_Executable' )

call s:ApplyDefaultSetting ( 'Python_InsertFileHeader', 'yes' )           " default: do insert a file header
call s:ApplyDefaultSetting ( 'Python_MapLeader', '' )                     " default: do not overwrite 'maplocalleader'
call s:ApplyDefaultSetting ( 'Python_Printheader', "%<%f%h%m%<  %=%{strftime('%x %H:%M')}     Page %N" )
call s:ApplyDefaultSetting ( 'Python_UseTool_make', 'yes' )
call s:ApplyDefaultSetting ( 'Xterm_Options', '-fa courier -fs 12 -geometry 80x24' )

let s:Python_GlobalTemplateFile = expand ( s:Python_GlobalTemplateFile )
let s:Python_LocalTemplateFile  = expand ( s:Python_LocalTemplateFile )
let s:Python_CustomTemplateFile = expand ( s:Python_CustomTemplateFile )
let s:Python_SnippetDir         = expand ( s:Python_SnippetDir )
...
function! s:EndOfLineComment () range
	...
	for line in range( a:lastline, a:firstline, -1 )
		...
		if getline(line) !~ '^\s*$'
			...
			exe 'normal! '.diff.'A '
			call mmtemplates#core#InsertTemplate (g:Python_Templates, 'Comments.end-of-line comment')
		endif
	endfor
	"
...
function! s:AdjustEndOfLineComm () range
	...
	let correct_idx = b:Python_LineEndCommentColumn
	"
...
function! s:SetEndOfLineCommPos () range
	...
	call s:ImportantMsg ( 'line end comments will start at column '.b:Python_LineEndCommentColumn )
	"
...
function! s:FunctionComment() range
	...
	"
	if fun_name != ''
		call mmtemplates#core#InsertTemplate ( g:Python_Templates, 'Comments.function description',
...
function! s:CodeSnippet ( action )
	...
	"
	" check directory
	if ! isdirectory( s:Python_SnippetDir )
		return s:ErrorMsg (
					\ 'Code snippet directory '.s:Python_SnippetDir.' does not exist.',
					\ '(Please create it.)' )
	endif
	"
	...
	if a:action == 'insert'
		...
		"
		" select file
		if has('browse') && s:Python_SnippetBrowser == 'gui'
			let snippetfile = browse ( 0, 'insert a code snippet', s:Python_SnippetDir, '' )
		else
			let snippetfile = input ( 'insert snippet ', s:Python_SnippetDir, 'file' )
		endif
		"
		" insert snippet
	...
	elseif a:action == 'create' || a:action == 'vcreate'
		...
		" select file
		if has('browse') && s:Python_SnippetBrowser == 'gui'
			let snippetfile = browse ( 1, 'create a code snippet', s:Python_SnippetDir, '' )
		else
			let snippetfile = input ( 'create snippet ', s:Python_SnippetDir, 'file' )
		endif
		"
		" create snippet
	...
	elseif a:action == 'view' || a:action == 'edit'
		...
		" select file
		if has('browse') && s:Python_SnippetBrowser == 'gui'
			let snippetfile = browse ( saving, a:action.' a code snippet', s:Python_SnippetDir, '' )
		else
			let snippetfile = input ( a:action.' snippet ', s:Python_SnippetDir, 'file' )
		endif
		"
		" open file
		if ! empty( snippetfile )
...
function! s:Run ( args )
	...
	if ! executable( s:Python_Executable )
		return s:ErrorMsg (
					\ 'Command "'.s:Python_Executable.'" not found. Not configured correctly?',
					\ 'Further information: :help g:Python_Executable' )
	endif

	" recognized errors:
	"  File "some.py", line 91, in <module>
	"  File "/.../other.py", line 334, in func
	let errformat = '%\\s%\\+File \"%f\"\, line %l\, %m'

	if s:Python_DirectRun == 'yes' && executable ( expand ( '%:p' ) )
		let exec   = expand ( '%:p' )
		let script = ''
	else
		let exec   = s:Python_Executable
		let script = shellescape ( expand ( '%' ) )
	endif

	let exec = s:ShellEscExec ( exec )

	if s:Python_OutputMethod == 'vim-io'

		" method : "vim - interactive"

		exe '!'.exec.' '.script.' '.a:args

	elseif s:Python_OutputMethod == 'vim-qf'

		" method : "vim - quickfix"

		" run script
		let python_output = system ( exec.' '.script.' '.a:args )
		...
		if v:shell_error == 0
			...
			silent exe 'cexpr python_output'
	...
	elseif s:Python_OutputMethod == 'buffer'

		" method : "buffer"

		if bufwinnr ( 'Python Output$' ) == -1
			" open buffer
			above new
			file Python\ Output
			...
			call Python_ResetMapLeader ()
		else
			" jump to window
			exe bufwinnr( 'Python Output$' ).'wincmd w'
		endif
	...
	elseif s:Python_OutputMethod == 'xterm'
...
function! s:Compile( mode ) range
	...
	if ! executable( s:Lua_CompilerExec )
		return s:ErrorMsg (
					...
					\ 'Further information: :help g:Python_CompilerExec' )
	endif
	"
	if a:mode == 'compile'
		let switch = fnamemodify( bufname('%'), ':p:r' )
"		if ! empty( g:Python_CompiledExtension )
"			let switch .= '.'.g:Python_CompiledExtension
"		endif
...
function! s:Hardcopy ( mode )
  ...
  if ! s:MSWIN && empty ( outfile )
	...
	"
	let &g:printheader = g:Python_Printheader
	"
	if s:MSWIN
...
"
"------------------------------------------------------------------------------
"  Python_SetMapLeader   {{{2
"------------------------------------------------------------------------------
function! Python_SetMapLeader ()
	if exists ( 'g:Python_MapLeader' )
		call mmtemplates#core#SetMapleader ( g:Python_MapLeader )
	endif
endfunction    " ----------  end of function Python_SetMapLeader  ----------
"
"------------------------------------------------------------------------------
"  Python_ResetMapLeader   {{{2
"------------------------------------------------------------------------------
function! Python_ResetMapLeader ()
	if exists ( 'g:Python_MapLeader' )
		call mmtemplates#core#ResetMapleader ()
	endif
endfunction    " ----------  end of function Python_ResetMapLeader  ----------
" }}}2
"
"-------------------------------------------------------------------------------
...
function! s:SetupTemplates()
	...
	"-------------------------------------------------------------------------------
	let g:Python_Templates = mmtemplates#core#NewLibrary ( 'api_version', '1.0' )
	"
	" mapleader
	if empty ( g:Python_MapLeader )
		call mmtemplates#core#Resource ( g:Python_Templates, 'set', 'property', 'Templates::Mapleader', '\' )
	else
		call mmtemplates#core#Resource ( g:Python_Templates, 'set', 'property', 'Templates::Mapleader', g:Python_MapLeader )
	endif
	"
	" some metainfo
	call mmtemplates#core#Resource ( g:Python_Templates, 'set', 'property', 'Templates::Wizard::PluginName',   'Python' )
	call mmtemplates#core#Resource ( g:Python_Templates, 'set', 'property', 'Templates::Wizard::FiletypeName', 'Python' )
	call mmtemplates#core#Resource ( g:Python_Templates, 'set', 'property', 'Templates::Wizard::FileCustomNoPersonal',   s:plugin_dir.'/python-support/rc/custom.templates' )
	call mmtemplates#core#Resource ( g:Python_Templates, 'set', 'property', 'Templates::Wizard::FileCustomWithPersonal', s:plugin_dir.'/python-support/rc/custom_with_personal.templates' )
	call mmtemplates#core#Resource ( g:Python_Templates, 'set', 'property', 'Templates::Wizard::FilePersonal',           s:plugin_dir.'/python-support/rc/personal.templates' )
	call mmtemplates#core#Resource ( g:Python_Templates, 'set', 'property', 'Templates::Wizard::CustomFileVariable',     'g:Python_CustomTemplateFile' )
	"
	" maps: special operations
	call mmtemplates#core#Resource ( g:Python_Templates, 'set', 'property', 'Templates::RereadTemplates::Map', 'ntr' )
	call mmtemplates#core#Resource ( g:Python_Templates, 'set', 'property', 'Templates::ChooseStyle::Map',     'nts' )
	call mmtemplates#core#Resource ( g:Python_Templates, 'set', 'property', 'Templates::SetupWizard::Map',     'ntw' )
	"
	" syntax: comments
	call mmtemplates#core#ChangeSyntax ( g:Python_Templates, 'comment', '§' )
	"
	"-------------------------------------------------------------------------------
	...
	if s:installation == 'system'
		call mmtemplates#core#ReadTemplates ( g:Python_Templates, 'load', s:Python_GlobalTemplateFile,
					\ 'name', 'global', 'map', 'ntg' )
	endif
	"
	" local templates (optional for global installation)
	if s:installation == 'system'
		call mmtemplates#core#ReadTemplates ( g:Python_Templates, 'load', s:Python_LocalTemplateFile,
	...
	if ! empty ( s:Python_AdditionalTemplates )
		call mmtemplates#core#AddCustomTemplateFiles ( g:Python_Templates, s:Python_AdditionalTemplates, "Python's additional templates" )
	endif
	"
	" personal templates (shared across template libraries) (optional, existence of file checked by template engine)
	call mmtemplates#core#ReadTemplates ( g:Python_Templates, 'personalization',
				\ 'name', 'personal', 'map', 'ntp' )
	"
	" custom templates (optional, existence of file checked by template engine)
	call mmtemplates#core#ReadTemplates ( g:Python_Templates, 'load', s:Python_CustomTemplateFile,
...
function! s:InsertFileHeader ()
	"
	if ! exists ( 'g:Python_Templates' )
		return
	endif
	"
	if g:Python_InsertFileHeader == 'yes'
		call mmtemplates#core#InsertTemplate ( g:Python_Templates, 'Comments.file description' )
	endif
	"
...
"
"-------------------------------------------------------------------------------
" s:SetExecutable : Set s:Python_Executable or s:Lua_CompilerExec   {{{1
"-------------------------------------------------------------------------------
"
function! s:SetExecutable ( exe_type, new_exec )
	"
	if a:exe_type == 'exe'
		let var = 's:Python_Executable'
...
function! s:GetOutputMethodList (...)
	return join ( s:Python_OutputMethodList, "\n" )
endfunction    " ----------  end of function s:GetOutputMethodList  ----------
"
"-------------------------------------------------------------------------------
" s:SetOutputMethod : Set s:Python_OutputMethod   {{{1
"-------------------------------------------------------------------------------
"
function! s:SetOutputMethod ( method )
	"
	if a:method == ''
		echo s:Python_OutputMethod
		return
	endif
	"
	" 'method' gives the output method
	if index ( s:Python_OutputMethodList, a:method ) == -1
		return s:ErrorMsg ( 'Invalid option for output method: "'.a:method.'".' )
	endif
	"
	let s:Python_OutputMethod = a:method
	"
	...
	"
	exe 'aunmenu '.s:Python_RootMenu.'.Run.output\ method.Output\ Method'
	"
	if s:Python_OutputMethod == 'vim-io'
		let current = 'vim\ io'
	elseif s:Python_OutputMethod == 'vim-qf'
	...
	"
	exe 'anoremenu ...400 '.s:Python_RootMenu.'.Run.output\ method.Output\ Method<TAB>(current\:\ '.current.') :echo "This is a menu header."<CR>'
	"
...
"
"-------------------------------------------------------------------------------
" s:SetDirectRun : Set s:Python_DirectRun   {{{1
"-------------------------------------------------------------------------------
"
function! s:SetDirectRun ( option )
	"
	if a:option == ''
		echo s:Python_DirectRun
		return
	endif
	"
	...
	"
	let s:Python_DirectRun = a:option
	"
	" update the menu header
	if ! has ( 'menu' ) || s:MenuVisible == 0
		return
	...
	"
	exe 'aunmenu '.s:Python_RootMenu.'.Run.direct\ run.Direct\ Run'
	"
	let current = s:Python_DirectRun
	"
	exe 'anoremenu ...400 '.s:Python_RootMenu.'.Run.direct\ run.Direct\ Run<TAB>(currently\:\ '.current.') :echo "This is a menu header."<CR>'
	"
...
function! s:CreateMaps ()
	"
	"-------------------------------------------------------------------------------
	" user defined commands (only working in Python buffers)
	"-------------------------------------------------------------------------------
	"
	command! -nargs=* -buffer -complete=file Python          call <SID>Run(<q-args>)
	command! -nargs=0 -buffer -complete=file LuaCompile   call <SID>Compile('compile')
	command! -nargs=0 -buffer -complete=file PythonCheck     call <SID>Compile('check')
	"
	"-------------------------------------------------------------------------------
	...
	if ! empty ( g:Python_MapLeader )
		...
		let g:maplocalleader = g:Python_MapLeader
	endif
	"
	"-------------------------------------------------------------------------------
	" comments
	...
	" output method
	"-------------------------------------------------------------------------------
	nnoremap    <buffer>            <LocalLeader>ro         :PythonOutputMethod<SPACE>
	inoremap    <buffer>            <LocalLeader>ro    <Esc>:PythonOutputMethod<SPACE>
	vnoremap    <buffer>            <LocalLeader>ro    <Esc>:PythonOutputMethod<SPACE>
	nnoremap    <buffer>            <LocalLeader>rd         :PythonDirectRun<SPACE>
	inoremap    <buffer>            <LocalLeader>rd    <Esc>:PythonDirectRun<SPACE>
	vnoremap    <buffer>            <LocalLeader>rd    <Esc>:PythonDirectRun<SPACE>
	nnoremap    <buffer>            <LocalLeader>rse        :PythonExecutable<SPACE>
	inoremap    <buffer>            <LocalLeader>rse   <Esc>:PythonExecutable<SPACE>
	vnoremap    <buffer>            <LocalLeader>rse   <Esc>:PythonExecutable<SPACE>
	...
	" settings
	"-------------------------------------------------------------------------------
	nnoremap    <buffer>  <silent>  <LocalLeader>rs         :call Python_Settings(0)<CR>
	inoremap    <buffer>  <silent>  <LocalLeader>rs    <Esc>:call Python_Settings(0)<CR>
	vnoremap    <buffer>  <silent>  <LocalLeader>rs    <Esc>:call Python_Settings(0)<CR>
	"
	"-------------------------------------------------------------------------------
	" help
	"-------------------------------------------------------------------------------
	...
	" toolbox
	"-------------------------------------------------------------------------------
	if s:Python_UseToolbox == 'yes'
		call mmtoolbox#tools#AddMaps ( s:Python_Toolbox )
	endif
	"
	"-------------------------------------------------------------------------------
	...
	" templates
	"-------------------------------------------------------------------------------
	call mmtemplates#core#CreateMaps ( 'g:Python_Templates', g:Python_MapLeader, 'do_special_maps', 'do_jump_map', 'do_del_opt_map' )
	"
...
function! s:InitMenus()
	...
	"
	" preparation
	call mmtemplates#core#CreateMenus ( 'g:Python_Templates', s:Python_RootMenu, 'do_reset' )
	"
	" get the mapleader (correctly escaped)
	let [ esc_mapl, err ] = mmtemplates#core#Resource ( g:Python_Templates, 'escaped_mapleader' )
	"
	exe 'anoremenu '.s:Python_RootMenu.'.Python  <Nop>'
	exe 'anoremenu '.s:Python_RootMenu.'.-Sep00- <Nop>'
	"
	"-------------------------------------------------------------------------------
	" menu headers
	"-------------------------------------------------------------------------------
	"
	call mmtemplates#core#CreateMenus ( 'g:Python_Templates', s:Python_RootMenu, 'sub_menu', '&Comments', 'priority', 500 )
	" the other, automatically created menus go here; their priority is the standard priority 500
	call mmtemplates#core#CreateMenus ( 'g:Python_Templates', s:Python_RootMenu, 'sub_menu', 'S&nippets', 'priority', 600 )
	call mmtemplates#core#CreateMenus ( 'g:Python_Templates', s:Python_RootMenu, 'sub_menu', '&Run'     , 'priority', 700 )
	if s:Python_UseToolbox == 'yes' && mmtoolbox#tools#Property ( s:Python_Toolbox, 'empty-menu' ) == 0
		call mmtemplates#core#CreateMenus ( 'g:Python_Templates', s:Python_RootMenu, 'sub_menu', '&Tool\ Box', 'priority', 800 )
	endif
	call mmtemplates#core#CreateMenus ( 'g:Python_Templates', s:Python_RootMenu, 'sub_menu', '&Help'    , 'priority', 900 )
	"
	"-------------------------------------------------------------------------------
	" comments
	"-------------------------------------------------------------------------------
	"
	let ahead = 'anoremenu <silent> '.s:Python_RootMenu.'.Comments.'
	let vhead = 'vnoremenu <silent> '.s:Python_RootMenu.'.Comments.'
	"
	...
	call mmtemplates#core#CreateMenus ( 'g:Python_Templates', s:Python_RootMenu, 'do_templates' )
	...
	"-------------------------------------------------------------------------------
	"
	let ahead = 'anoremenu <silent> '.s:Python_RootMenu.'.Snippets.'
	let vhead = 'vnoremenu <silent> '.s:Python_RootMenu.'.Snippets.'
	"
	...
	call mmtemplates#core#CreateMenus ( 'g:Python_Templates', s:Python_RootMenu, 'do_specials',
	...
	"-------------------------------------------------------------------------------
	"
	let ahead = 'anoremenu          '.s:Python_RootMenu.'.Run.'
	let shead = 'anoremenu <silent> '.s:Python_RootMenu.'.Run.'
	let vhead = 'vnoremenu <silent> '.s:Python_RootMenu.'.Run.'
	"
	...
	"
	exe shead.'&buffer\ "Python\ Output".buffer\ "Python\ Output"  :echo "This is a menu header."<CR>'
	exe shead.'&buffer\ "Python\ Output".-SepHead-                 :'
	exe shead.'&buffer\ "Python\ Output".load\ into\ quick&fix<TAB>'.esc_mapl.'qf               :call <SID>OutputBufferErrors(0)<CR>'
	exe shead.'&buffer\ "Python\ Output".qf\.\ and\ &jump\ to\ first\ error<TAB>'.esc_mapl.'qj  :call <SID>OutputBufferErrors(1)<CR>'
	"
	exe shead.'-Sep01-                                   :'
	"
	...
	"
	exe ahead.'&set\ executable<TAB>'.esc_mapl.'rse                :PythonExecutable<SPACE>'
	exe ahead.'&set\ compiler\ exec\.<TAB>'.esc_mapl.'rsc          :PythonCompilerExec<SPACE>'
	exe shead.'-Sep02-                                             :'
	"
	...
	exe shead.'-Sep03-                                             :'
	"
	exe shead.'&settings<TAB>'.esc_mapl.'rs  :call Python_Settings(0)<CR>'
	"
	" run -> output method
	"
	...
	exe shead.'direct\ run.&no<TAB>always\ use\ :PythonExecutable   :call <SID>SetDirectRun("no")<CR>'
	"
	" deletes the dummy menu header and displays the current options
	" in the menu header of the sub-menus
	call s:SetOutputMethod ( s:Python_OutputMethod )
	call s:SetDirectRun ( s:Python_DirectRun )
	"
	"-------------------------------------------------------------------------------
	" tool box
	"-------------------------------------------------------------------------------
	"
	if s:Python_UseToolbox == 'yes' && mmtoolbox#tools#Property ( s:Python_Toolbox, 'empty-menu' ) == 0
		call mmtoolbox#tools#AddMenus ( s:Python_Toolbox, s:Python_RootMenu.'.&Tool\ Box' )
	endif
	"
	"-------------------------------------------------------------------------------
	" help
	"-------------------------------------------------------------------------------
	"
	let ahead = 'anoremenu <silent> '.s:Python_RootMenu.'.Help.'
	let vhead = 'vnoremenu <silent> '.s:Python_RootMenu.'.Help.'
	"
	exe ahead.'-Sep01-                                  :'
	exe ahead.'&help\ (Python-Support)<TAB>'.esc_mapl.'hs  :call <SID>HelpPlugin()<CR>'
	"
...
function! s:ToolMenu( action )
	...
	if a:action == 'setup'
		...
		anoremenu <silent> 40.1162 &Tools.Load\ Python\ Support   :call <SID>AddMenus()<CR>
	elseif a:action == 'load'
		aunmenu   <silent> &Tools.Load\ Python\ Support
		anoremenu <silent> 40.1162 &Tools.Unload\ Python\ Support :call <SID>RemoveMenus()<CR>
	elseif a:action == 'unload'
		aunmenu   <silent> &Tools.Unload\ Python\ Support
		anoremenu <silent> 40.1162 &Tools.Load\ Python\ Support   :call <SID>AddMenus()<CR>
		exe 'aunmenu <silent> '.s:Python_RootMenu
	endif
	"
...
"
"-------------------------------------------------------------------------------
" Python_Settings : Print the settings on the command line.   {{{1
"-------------------------------------------------------------------------------
"
function! Python_Settings( verbose )
	"
	if     s:MSWIN | let sys_name = 'Windows'
	elseif s:UNIX  | let sys_name = 'UN*X'
	else           | let sys_name = 'unknown' | endif
	"
	let python_exe_status = executable( s:Python_Executable ) ? '' : ' (not executable)'
	...
	if exists ( 'g:Python_Templates' )
		let [ templ_style, msg ] = mmtemplates#core#Resource( g:Python_Templates, 'style' )
		"
		let txt .=
					\  '                   author :  "'.mmtemplates#core#ExpandText( g:Python_Templates, '|AUTHOR|'       )."\"\n"
					\ .'                authorref :  "'.mmtemplates#core#ExpandText( g:Python_Templates, '|AUTHORREF|'    )."\"\n"
					\ .'                    email :  "'.mmtemplates#core#ExpandText( g:Python_Templates, '|EMAIL|'        )."\"\n"
					\ .'             organization :  "'.mmtemplates#core#ExpandText( g:Python_Templates, '|ORGANIZATION|' )."\"\n"
					\ .'         copyright holder :  "'.mmtemplates#core#ExpandText( g:Python_Templates, '|COPYRIGHT|'    )."\"\n"
					\ .'                  licence :  "'.mmtemplates#core#ExpandText( g:Python_Templates, '|LICENSE|'      )."\"\n"
	...
	" toolbox
	if s:Python_UseToolbox == 'yes'
		let toollist = mmtoolbox#tools#GetList ( s:Python_Toolbox )
		if empty ( toollist )
	...
	" templates, snippets
	if exists ( 'g:Python_Templates' )
		let [ templist, msg ] = mmtemplates#core#Resource ( g:Python_Templates, 'template_list' )
		let sep  = "\n"."                             "
	...
	let txt .=
				\  '       code snippets dir. :  '.s:Python_SnippetDir."\n"
	if a:verbose >= 1
		let	txt .= "\n"
					\ .'                mapleader :  "'.g:Python_MapLeader."\"\n"
					\ .'               load menus :  "'.s:Python_LoadMenus."\"\n"
					\ .'       insert file header :  "'.g:Python_InsertFileHeader."\"\n"
	endif
	let txt .= "\n"
	let txt .=
				\  '     python (interpreter) :  '.s:Python_Executable.python_exe_status."\n"
				\ .'          luac (compiler) :  '.s:Lua_CompilerExec.luac_exe_status."\n"
	" various settings, maps, menus, running, compiling, ...
	if a:verbose >= 1
		let	txt .=
					\  '            output method :  "'.s:Python_OutputMethod."\"\n"
					\ .'               direct run :  "'.s:Python_DirectRun."\"\n"
	endif
	" xterm (UNIX only)
	...
	let txt .=
				...
				\ ." Python-Support, Version ".g:Python_Version." / Wolfgang Mehner / wolfgang-mehner@web.de\n\n"
	"
	if a:verbose == 2
		split PythonSupport_Settings.txt
		put = txt
	else
		echo txt
	endif
...
if s:Python_UseToolbox == 'yes'
	...
	let s:Python_Toolbox = mmtoolbox#tools#NewToolbox ( 'Python' )
	call mmtoolbox#tools#Property ( s:Python_Toolbox, 'mapleader', g:Python_MapLeader )

	call mmtoolbox#tools#Load ( s:Python_Toolbox, s:Python_ToolboxDir )

	" debugging only:
	"call mmtoolbox#tools#Info ( s:Python_Toolbox )
...
if s:Python_LoadMenus == 'startup'
	call s:AddMenus ()
endif

" user defined commands (working everywhere)
command! -nargs=? -complete=custom,<SID>GetOutputMethodList PythonOutputMethod   call <SID>SetOutputMethod(<q-args>)
command! -nargs=? -complete=custom,<SID>GetDirectRunList    PythonDirectRun      call <SID>SetDirectRun(<q-args>)
command! -nargs=? -complete=shellcmd                        PythonExecutable     call <SID>SetExecutable('exe',<q-args>)
command! -nargs=? -complete=shellcmd                        PythonCompilerExec   call <SID>SetExecutable('compile',<q-args>)

if has( 'autocmd' )
	autocmd FileType *
				\	if &filetype == 'python' && ! exists( 'g:Python_Templates' ) |
				\		if s:Python_LoadMenus == 'auto' | call s:AddMenus () |
	...
	autocmd FileType *
				...
				\	endif
	autocmd BufNewFile  *.python  call s:InsertFileHeader()
endif
" }}}1
"-------------------------------------------------------------------------------
